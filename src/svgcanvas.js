...
canvas.undoMgr = new svgedit.history.UndoManager({
        handleHistoryEvent: function(eventType, cmd) {
                ...
                if (eventType == EventTypes.BEFORE_UNAPPLY || eventType == EventTypes.BEFORE_APPLY) {
                        ...
                        call("changed", elems);
                        // Note with something has been adjusted other than select (new element)
                        console.log("Something was undone! <<<<<");
                        Session.set("last-updated", $.now());
...
//                                         newText.textContent = "text";
		...
		// WSL: Mark the continuous poll position for all actions 
		//console.log("Update Cont (Poll Mode)"); 
                switch (current_mode)
                {
                        case "select":
...
//                                                        }
				...
				// WSL: Mark while the pencil is drawing
				/*
				console.log("M");
				runExtensions("partialDraw", {
                        		event: evt,
                        		mouse_x: mouse_x,
                        		mouse_y: mouse_y,
                        		selected: selected
...
this.deleteSelectedElements = function() {
	this.deleteElementsByID(selectedElements);
}

// WSL::
// Function: deleteElementsByID
// Removes all selected elements from the DOM and adds the change to the 
// history stack
this.deleteElementsByID = function(inElements) {
/*
console.log(inElements)
console.log(inElements[0])
console.log("=====")
*/
        var batchCmd = new BatchCommand("Delete Elements");
        var len = inElements.length;
        var selectedCopy = []; //selectedElements is being deleted
        for (var i = 0; i < len; ++i) {
//console.log(inElements[i])
                var selected = inElements[i];
                if (selected == null) break;
...
this.moveToTopSelectedElement = function() {
        this.moveToTopByElementID(selectedElements.filter(Boolean));
};

// WSL::
// Function: moveToTopByElementID
// Repositions the selected element to the bottom in the DOM to appear on top of
// other elements
this.moveToTopByElementID = function(elemAry) {
/*
        // NOTE: Code for getting the element set by ID string
        elemAry = new Array();
        $.each(idArray, function(key, val) {
                // Create an array of targetd elements
                elemAry.push(getElem(val));
        });
*/

        var selected = elemAry.reverse();
        var batchCmd = new BatchCommand("Move ID to top");
	...
	// WSL: Throw changed event on layer change
	runExtensions("layerChanged", {
		//event: evt,
		move: "top",
		selected: elemAry
         });
...
this.moveToBottomSelectedElement = function() {
        this.moveToBottomByElementID(selectedElements.filter(Boolean));
};

// Function: moveToBottomByElementID
// Repositions the selected element to the top in the DOM to appear under 
// other elements
this.moveToBottomByElementID = function(elemAry) {
        var selected = elemAry.reverse();
...
this.moveUpDownSelected = function(dir) {
        this.moveUpDownByID(selectedElements.filter(Boolean), dir);
};

// Function: moveUpDownByID
// Moves the select element up or down the stack, based on the visibly
// intersecting elements
//
// Parameters: 
// dir - String that's either 'Up' or 'Down'
this.moveUpDownByID = function(elemAry, dir) {
        var selected = elemAry;
...
this.cloneSelectedElements = function(x,y, drag) {
        // Note with something has been adjusted other than select (new element)
        console.log("Something was cloned! +++++");
        Session.set("last-updated", $.now());
